...
async def create_data_source(
    ...
    size_info = await SizeTrackingService.calculate_initial_size(
        data_source.source_type,
        data_source.connection_settings
    )
    
    # Create data source with size information
    db_data_source = DataSource(
        user_id=current_user.id,
        name=data_source.name,
        source_type=data_source.source_type,
        connection_settings=data_source.connection_settings,
        raw_size_bytes=size_info["raw_size_bytes"],
        document_count=size_info["document_count"]
    )
...
async def upload_file(
    ...
    data_source.raw_size_bytes = size_info["raw_size_bytes"]
    data_source.document_count = size_info["document_count"]
